1. `bi` 是独立安装单元，运行时不要依赖其他技能包共享口径；行动规则、原则、模板和脚本都放在同一个 skill 内。
2. 项目稳定需求和架构考量沉淀到 `byissue/spec/`，复杂操作经验沉淀到 `byissue/notes/`；几乎每次启动都必须知道的短规则直接写入项目已有 `AGENTS.md` 或 `CLAUDE.md`，也不要在两个指令文件里重复。
