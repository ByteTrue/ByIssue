...
- **两段路由**：技能开不开是第一段，由宿主对 frontmatter `description` 做纯文本匹配决定；选哪个姿态是第二段，由 `SKILL.md` 的姿态表决定。两套触发词分别维护，checker 断言每个姿态至少有一个触发词出现在 description 里。description 里的条件式触发（「当项目存在某文件时」）无效。
...
- **否定式表述要挣得它的注意力成本。** 一条「不要 X」只有在 X 是模型的强默认行为时才值得写（不自动 commit、不预读未命中的姿态文件、不用槽位凑模板）。挡一个模型本来不会做的事，既占上下文又把那件事送进注意力——「不要想大象」。预防性禁令尤其容易长：2026-09-21 清掉的「无边界全仓审计」在五个文件里各禁一次，而模型的默认恰恰相反，是过度聚焦当前文件。
- **`SKILL.md` 永远在上下文里，所以它已有的规则不要在 reference 里重述。** 这是单一归属原则用在否定式表述上的形式。reference 之间是按需加载的，彼此不能假定对方在场；`SKILL.md` 可以。
