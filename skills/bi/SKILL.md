...
ByIssue 是一套理解和推进软件演化的方法。它用 Vision、Project Spec、Epic、Issue 和 `byissue/` 中的制度记忆连接目标、现实与行动；它**不是**强制流水线，也不是 Agent 编排器。

调用 `bi` 不等于跑完整流程。先判断用户此刻要讨论、理解、设计、实现还是收尾，再采用匹配的管理强度。
...
8. 写或修改 Agent 技能本身，不属于 ByIssue 的职责。
9. **姿态在会话中持续；被授权的单个动作不改变它。** 用户在讨论里批准了一个要落盘的动作，做完就回到讨论——动手过一次不等于整场会话转成执行。切换姿态要么来自用户的新信号，要么你明说「我切到 X 姿态」并得到同意。
...
用户明确说「不要痕迹 / 别写 issue / 别落盘 ff」时：

- **不创建任何事项**，包括 `ff`；也不自动改 Vision；默认不 commit。
- **仍然跑开工协议、仍然验证。** 省掉的只是痕迹，不是判断和证据。
- **真相失效仍须处理**：同步相关 project spec，或明确标记漂移并告诉用户。
- 这条**不改变姿态**。「做这个 issue，但别留痕迹」仍然是受管理实现，走 [do](references/do.md)，只是不落 `ff`；它不会因为出现「不要痕迹」就变成快改。

这是这条例外的唯一出处，各姿态文件只链接过来，不重述。
...
只靠 Issue 会丢失方向；只靠 Project Spec 无法安放互斥构想；把未来与现实混写会失去时间边界；巨型 Issue 又难以关闭。因此，只把**值得跨会话保留**的信息写入 `byissue/`，并放到承担相应职责的位置。

### `byissue/` 工作区地图

ByIssue 的制度记忆统一存放在项目的 `byissue/` 下。
...
每棵编号树分别编号：根 `issues/`、每个 Epic 的 `issues/`、`epics/`、`notes/`、`talks/` 都从本树已有最大开头数字加 1。编号至少三位；超过 999 后继续使用 `1000`、`1001`，不设上限。

Issue 编号不是全局身份，同一棵树里也可能有多个事项共用同一个编号——并行会话各自取「最大序号 + 1」就会撞，这是已接受的代价。因此**引用任何事项都必须给完整文件名或目录名**（`issues/018-x-feishu-first-class-entry/`），不要写裸编号（`Issue 018`）。跨 Epic 引用再补完整路径，或写成 `Epic NNN / Issue NNN` 并附文件名。
...
| 受管理实现 | 推进到完成即可；收尾汇报给出关闭建议，用户授权提交时与代码合并为单次 commit（见 [do](references/do.md)） |
| 用户说“做完”或“修好” | 完成实现与验证；小改仍留下 `ff`；收尾汇报给出关闭建议，授权提交时合并完成 |
...
在设计、实现、快改、修 bug、维护规格或穿刺之前，如果项目已有 `byissue/`，先完成以下动作。各 reference 不再重复展开这套协议。

1. **浏览路径并检索关键词。** 扫描 `byissue/` 的结构与当前主题；同一会话中已经扫描且没有新写入时，复用已有结果。
2. **按权重深入读取。** 先读 `spec/`，再读相关 Epic 与 Notes，然后递归检索根目录和各 Epic 的 issues 树，包括 `-x-`、`-d-` 与 `ff`；最后按需读取 Talks、Vision 和 Tools。
...
把 `byissue/` 当作制度记忆：遇到奇怪代码先查 Spec，踩到旧坑先查 Notes，要理解历史取舍先查 Issue。
...
实体模板位于 `templates/entities/`，包括 `issue.md`、`ff-issue.md` 以及 Explore、Vision、Spec、Talk、Note 等模板。初始化脚本是 `scripts/init_byissue.py`。产物格式以相应 reference 为准，不要只根据文件名猜测。
